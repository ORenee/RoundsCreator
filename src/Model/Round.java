...
public class Round {
    ...
    /**
     * Returns a string array including the heat number, the dance name, and the dancers
     * in lead & follow, lead & follow format
     * @param startingHeatNumber
     * @return
     */
    public String[][] formatForDisplay(int startingHeatNumber){
        // height is the number of dances. Since a heat could be empty,
        // we have to recalculate the number of dances that we have
        int height = 0;
        if(dance1 != null) { height += 1; }
        ...
        if(dance4 != null) { height += 1; }
        if(dance5 != null) { height += 1; }
        String[][] results = new String[height][3]; // heat number, dance name, dancers

        for(int row = 0; row < height; ++row){
            results[row][0] = "" + (startingHeatNumber + row); // if we start heats at #1, first heat number is 1 + 0
        }

        // input the dance names
        int position = 0;
        if(dance1 != null){
            results[position][1] = dance1.getDance().getName();
            position += 1;
        }
        if(dance2 != null){
            results[position][1] = dance2.getDance().getName();
            position += 1;
        }
        if(dance3 != null){
            results[position][1] = dance3.getDance().getName();
            position += 1;
        }
        if(dance4 != null){
            results[position][1] = dance4.getDance().getName();
            position += 1;
        }
        if(dance5 != null){
            results[position][1] = dance5.getDance().getName();
        }

        // input the names of dancers
        position = 0;
        if(dance1 != null){
            results[position][2] = dance1.getCouples()
                    .stream()
                    .map(x -> x.getLead() + " & " + x.getFollow())
                    .collect(Collectors.joining(", "));;
            position += 1;
        }
        if(dance2 != null){
            results[position][2] = dance2.getCouples()
                    .stream()
                    .map(x -> x.getLead() + " & " + x.getFollow())
                    .collect(Collectors.joining(", "));;
            position += 1;
        ...
        if(dance3 != null){
            results[position][2] = dance3.getCouples()
                    .stream()
                    .map(x -> x.getLead() + " & " + x.getFollow())
                    .collect(Collectors.joining(", "));;
            position += 1;
        ...
        if(dance4 != null){
            results[position][2] = dance4.getCouples()
                    .stream()
                    .map(x -> x.getLead() + " & " + x.getFollow())
                    .collect(Collectors.joining(", "));;
            position += 1;
        ...
        if(dance5 != null){
            results[position][2] = dance5.getCouples()
                    .stream()
                    .map(x -> x.getLead() + " & " + x.getFollow())
                    .collect(Collectors.joining(", "));;
        }
    ...
    public Round addToRoundWhereOnlyOneCouple(Round newRound){
        ...
        if(newRound.dance1 != null) {
            if (newRound.dance1.getCouples().size() == 1 && this.dance1.getCouples().size() < maxOnfloor) {
                // make sure they're not already dancing in this round
                // can use getfirst() because we know there is only one couple
                if(!includesDancers(newRound.dance1.getCouples().getFirst(), false)){
        ...
        if(newRound.dance2 != null) {
            if (newRound.dance2.getCouples().size() == 1 && this.dance2.getCouples().size() < maxOnfloor) {
                if(!includesDancers(newRound.dance2.getCouples().getFirst(), false)) {
        ...
        if(newRound.dance3 != null) {
            if (newRound.dance3.getCouples().size() == 1 && this.dance3.getCouples().size() < maxOnfloor) {
                if(!includesDancers(newRound.dance3.getCouples().getFirst(), false)) {
        ...
        if(newRound.dance4 != null){
            if(newRound.dance4.getCouples().size() == 1 && this.dance4.getCouples().size() < maxOnfloor){
                if(!includesDancers(newRound.dance4.getCouples().getFirst(), false)) {
        ...
        if(newRound.dance5 != null){
            if(newRound.dance5.getCouples().size() == 1 && this.dance5.getCouples().size() < maxOnfloor){
                if(!includesDancers(newRound.dance5.getCouples().getFirst(), false)) {
